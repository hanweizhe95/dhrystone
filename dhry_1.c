/*       gcc dhry_1.c dhry_2.c -m64 -lrt -lc -lm -o dhry2 
 *************************************************************************
 *
 ...
 */
 ...
 #include "dhry.h"
 // #include "cpuidh.h" <-- Removed dependency
...
int             Int_Glob;
 ...
 /* Variables for time measurement previously provided by cpuidh.h */
 double         secs;
 char           timeday[100];
 ...
 void end_time() {
     ...
     secs = (end_tv.tv_sec - start_tv.tv_sec) + (end_tv.tv_usec - start_tv.tv_usec) / 1000000.0;
 }

 void local_time() {
     time_t now = time(NULL);
     struct tm *t = localtime(&now);
     strftime(timeday, sizeof(timeday), "%Y-%m-%d %H:%M:%S", t);
 }
 ...
 ***********************************************************************/
   ...
   Arr_2_Glob [8][7] = 10;
    ...
    // getDetails(); <-- Removed call to undefined function
    // for (i=1; i<10; i++)
    // {
    //     printf("%s\n", configdata[i]);
    // }
